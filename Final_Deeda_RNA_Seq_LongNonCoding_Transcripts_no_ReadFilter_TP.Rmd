---
title: "Deeda RNASeq DESeq Analysis- Long Non-Coding Transcripts No Filtering"
...
##### Save Path For Data
save_path <- "/scratch/tporter/TB_20231117_Megans_DESeq/Data_LongNonCoding_Transcripts_No_Filter/"
prefix <- "Long_Non_Coding_Transcripts_No_Filter_"
...
We are going to filter all genes that have less than 10 reads in at least 10 samples.
```{r}
genes.anno %>%
  group_by(gene_type) %>%
  tally() %>%
...
lnc_coding <- genes.anno %>%
  filter(gene_type == "lncipedia_lncRNA")

lnc_coding <- lnc_coding$gene_id

TB_RNA <- TB_RNA[row.names(TB_RNA) %in% lnc_coding,]
