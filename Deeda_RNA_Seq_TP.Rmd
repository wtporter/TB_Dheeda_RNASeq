...
testset <- as.matrix(read.csv("/scratch/mfolkerts/Deeda_RNA_Seq/Latest_data/countTable_dedup_nocontrols.txt", sep="\t", row.names = "Geneid"))
samplecoldata <- read.csv("/scratch/mfolkerts/Deeda_RNA_Seq/Latest_data/metadata_RNASeq_Deeda.csv", row.names=1)
samplecoldata$Group <- factor(samplecoldata$Group)

rownames(samplecoldata) %in% colnames(testset)
```
...
DeedaRNASeq <- DESeq(DeedaRNASeq)

SignificantResults <- results(DeedaRNASeq, alpha=0.05)
